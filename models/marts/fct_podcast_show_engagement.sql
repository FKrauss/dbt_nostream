...
-- Daily engagement metrics at the show (podcast) level.
-- Groups all share and zap activity by show_guid and event_date.
...
WITH episode_events AS (
  ...
  WHERE show_guid IS NOT NULL
  GROUP BY show_guid, DATE(created_at)
),
...
SELECT
  COALESCE(ee.show_guid, ze.show_guid) AS show_guid,
